...
public class Main {
    ...
    public static boolean checkForPassword(String str, int minLength) {
        ...
        String lengthCheck = String.format("^.{%d,}$",  minLength);
        final boolean propertyOne = Pattern.matches(passwordRegex, str);
        final boolean propertyTwo = Pattern.matches(lengthCheck, str);
    ...
    public static List<String> extractEmails(String str) {
        ...
        final Pattern pattern = Pattern.compile("[a-zA-Z0-9_-]+@(mail\\.)?utoronto\\.ca");
    ...
    public static boolean checkForDoubles(String str) {
        ...
        for (String s : capitals.split("")) {
            if (str.matches(String.format("^.*%s.*%s.*$", s, s)))
                return true;
        }
        return false;
    }
